# iceDesk 26 [<img src="https://mirrors.lalaki.cn/icon.png" width="64" alt="iceDesk 26">](https://github.com/lalakii/iceDesk/releases)

[![Android: 9+ (shields.io)](https://img.shields.io/badge/Android-9+-2f9b45?logo=android)](https://mirrors.lalaki.cn) 
[![Downloads](https://img.shields.io/github/downloads/lalakii/iceDesk/total)](https://github.com/lalakii/iceDesk/releases)

一个简单的安卓 Launcher 应用，iceDesk 的核心功能是配合冰箱、小黑屋或 Shizuku 冻结、解冻 App。并且在冻结 App 之后，桌面上的图标不会消失！当前版本安装包大小 868 KB，支持 Android 9 或更高版本系统。

我决定等 iceDesk 大小超过 1MB 时就重构它, 是时候抛弃我古老的代码体系了！

## 为什么没有源码？
由于保管不善部分源码丢失，且加入了一些奇奇怪怪的编译依赖，导致环境配置困难，其他设备很难复现。不过主要还是代码稀烂，毕竟代码可追溯到2020年，那时的我还是一个青涩的萌新，对于写代码几乎是一窍不通，只能是不分享出来了。

## 为什么没有小部件、分页、文件夹归类等实现？
这个桌面最初是为我的备用机打造，不需要安装太多 App 和花里胡哨的功能，所以没有考虑做这些。

## 主要功能介绍（还有一些好玩的功能等你发现）

+ 单击桌面时钟组件，打开系统自带时钟App
+ 双击桌面时钟组件，进入iceDesk的设置界面
+ 长按桌面时钟组件，可进行图标排序，此时单击应用图标可以自定义应用名称、修改应用图标（从第三方图标包里选取图标）或是隐藏应用
+ 桌面时钟下滑展开通知栏，上划关闭屏幕（需要Shizuku）
+ 支持加载第三方图标包（icon pack）
+ 配合第三方应用冻结App时，图标不会从桌面消失
+ 侧滑应用列表功能，在屏幕一侧滑动可以快捷切换到指定App
+ 支持音量键唤醒屏幕（需要Shizuku）
+ 位于桌面时，按下音量-键可以隐藏桌面，双击音量-键恢复
+ 集成了发送/接收短信的功能，关键词拦截短信，短信转发至邮箱
+ 自动接听电话(营销电话，我接死你们~~)、来电静音
+ 集成了一个 Uperf 模块模式调节的选项, 支持自定义模式, 息屏时切换到自定义模式
+ 桌面上的"一键锁屏"图标，长按有几个便捷的菜单，进入桌面设置，打开浏览器等…

## 下载地址

[就在 Github 下载(推荐)](https://github.com/lalakii/iceDesk/releases) | [到 Lalaki 的网站下载](https://mirrors.lalaki.cn)

**下面是备用下载地址，注意：这些云盘可能需要注册登录才能下载，蓝奏云也无法直接下载 apk 了，请知悉**

[蓝奏云备用下载1](https://a01.lanzout.com/b00y5uzoj) | [蓝奏云备用下载2](https://a01.lanzoui.com/b00y5uzoj) | [蓝奏云备用下载3](https://a01.lanzouv.com/b00y5uzoj)
密码:2eb4

[123云盘备用下载](https://www.123865.com/s/jE3Sjv-37xxd)

## 屏幕截图
![iceDesk app](https://fastly.jsdelivr.net/gh/lalakii/iceDesk@master/demo.jpg)

![iceDesk app](https://fastly.jsdelivr.net/gh/lalakii/iceDesk@master/demo2.jpg)

## By lalaki.cn

[i@lalaki.cn](mailto:i@lalaki.cn)
